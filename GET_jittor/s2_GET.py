...
def train_dual(backbone, projector, model_tes,ln_t,train_loader, test_loader, unlabelled_train_loader, args,device):
    ...
    optimizer = jt.optim.SGD(list(ln_t.parameters())+list(projector.parameters())+list(backbone.parameters()), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # clustering, sup
            sup_logits_text = jt.concat([f[mask_lab] for f in (student_out_text / 0.1).chunk(2)], dim=0)
            sup_labels_text = jt.concat([class_labels[mask_lab] for _ in range(2)], dim=0)
            ...
            me_max_loss_text = - jt.sum(jt.log(avg_probs_text**(-avg_probs_text))) + math.log(float(len(avg_probs_text)))  # mean entropy reg for text
            ...
            contrastive_loss_text = nn.CrossEntropyLoss()(contrastive_logits_text, contrastive_labels_text)

            # representation learning, sup
            student_proj_text = jt.concat([f[mask_lab].unsqueeze(1) for f in student_proj_text.chunk(2)], dim=1)
            student_proj_text = student_proj_text/student_proj_text.norm(dim=-1,keepdim=True)
            ...
            # clustering, sup
            sup_logits = jt.concat([f[mask_lab] for f in (student_out / 0.1).chunk(2)], dim=0)
            sup_labels = jt.concat([class_labels[mask_lab] for _ in range(2)], dim=0)
            cls_loss = nn.CrossEntropyLoss()(sup_logits, sup_labels)

            # clustering, unsup
            cluster_loss = cluster_criterion(student_out, teacher_out, epoch)
            avg_probs = (student_out / 0.1).softmax(dim=1).mean(dim=0)
            me_max_loss = - jt.sum(jt.log(avg_probs**(-avg_probs))) + math.log(float(len(avg_probs))) # mean entropy reg for visual
            ...
            # representation learning, sup
            student_proj = jt.concat([f[mask_lab].unsqueeze(1) for f in student_proj.chunk(2)], dim=1)
            student_proj = student_proj/student_proj.norm(dim=-1,keepdim=True)
            ...
            ## -----CICO-----
            lab_idx = jt.unique(class_labels[mask_lab])
            class_labels_2 = jt.concat([class_labels,class_labels])
            mask_lab_2 = jt.concat([mask_lab,mask_lab])
            anchor_img = [image_features[mask_lab_2][jt.where(class_labels_2[mask_lab_2] == i)[0]].mean(0) \
                for i in lab_idx]
            anchor_img = jt.concat(anchor_img,dim=0).detach()
            if len(anchor_img.size()) == 1:
                anchor_img = anchor_img.unsqueeze(0)
            
            sim_img = nn.softmax(image_features/image_features.norm(dim=-1,keepdim=True) @ (anchor_img/anchor_img.norm(dim=-1,keepdim=True)).t() / 0.4, dim=1)

            sim_img_teacher = sim_img.detach()
            
            anchor_text = [text_features_ln[mask_lab_2][jt.where(class_labels_2[mask_lab_2] == i)[0]].mean(0) \
                for i in lab_idx]
            anchor_text = jt.concat(anchor_text,dim=0).detach()
            
            if len(anchor_text.size()) == 1:
                anchor_text = anchor_text.unsqueeze(0)
            
            sim_text = nn.softmax(text_features_ln/text_features_ln.norm(dim=-1,keepdim=True) @ (anchor_text/anchor_text.norm(dim=-1,keepdim=True)).t() / 0.4, dim=1)
            

            sim_text_teacher = sim_text.detach()

            kl_img = jt.mean(jt.sum(-jt.log(sim_img) * sim_text_teacher, dim=-1))
            kl_text = jt.mean(jt.sum(-jt.log(sim_text) * sim_img_teacher, dim=-1))
        ...
        jt.save(save_dict, args.model_path)
...
def test(backbone, projector, model_tes,ln_t, test_loader, epoch, save_name, args,device):
    ...
    for batch_idx, (images, label, _) in enumerate(tqdm(test_loader)):
        ...
        with jt.no_grad():
            ...
            # print(logits_mix,logits_mix.argmax(1))
            preds_img.append(logits_img.argmax(1)[0].cpu().numpy())
            
            preds_text.append(logits_text.argmax(1)[0].cpu().numpy())
            preds_mix.append(logits_mix.argmax(1)[0].cpu().numpy())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_name', default='S2_GET', type=str)
    ...
    YOUR_TES_CKPT = "dev_outputs/GET/log/debug/checkpoints/tes_model.pkl"
    state_dict = jt.load(YOUR_TES_CKPT)
    model_tes.projector.load_parameters(state_dict)
    ...
    # PROJECTION HEAD
    # ----------------------
    projector = DINOHead(in_dim=768, out_dim=args.mlp_out_dim, nlayers=args.num_mlp_layers)
    
    
    # make pseudo text embedding match the dim of visual embedding (except 512 to 512 for imagenet1k)
    ln_t = nn.Linear(in_features=512,out_features=768)
