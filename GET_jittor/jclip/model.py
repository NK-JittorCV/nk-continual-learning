...
from .simple_tokenizer import SimpleTokenizer  # new add (weg)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, input_resolution: int, patch_size: int, width: int,
                 layers: int, heads: int, output_dim: int,  use_proj: bool=True):
        super().__init__()
...
class TES_CLIP(nn.Module):
    ...
    def __init__(
        ...
        self.tokenizer = SimpleTokenizer()  # new add (weg)
        self.sot_token = self.tokenizer.encoder["<|startoftext|>"]  # new add (weg)
        self.eot_token = self.tokenizer.encoder["<|endoftext|>"]  # new add (weg)
    ...
    def build_attention_mask(self):
        mask = jt.empty((self.n_ctx, self.n_ctx))
    ...
    def encode_pseudo_text(self,x,end_token_ids,text_pe=True):
        # x = x.type(self.dtype)
        # add positional embedding
        if text_pe:
            x = x+self.positional_embedding[:x.shape[1]]# [bs,9,512]
        ...
        out = x[jt.arange(x.shape[0]), end_token_ids] @ self.text_projection
        return out

    def prepare_pseudo_text_tensor(self, pseudo_tokens, valid_mask=None, w_dropout=False):
        # device = pseudo_tokens.device
        num_preds, num_words, word_dim = pseudo_tokens.shape # bs,7,512
        sot_token = self.token_embedding(jt.Var([self.sot_token])) 
        eot_token = self.token_embedding(jt.Var([self.eot_token]))
  
        sot_token = sot_token.view(1, 1, word_dim).repeat(num_preds, 1, 1)
        eot_token = eot_token.view(1, 1, word_dim).repeat(num_preds, 1, 1)
        pseudo_tokens = jt.concat([sot_token, pseudo_tokens, eot_token], dim=1) # [bs,9,512]
        num_words += 2
        # words drop out
        if w_dropout:
            ...
            empty_token = self.token_embedding(jt.Var([0]
                                                        ))
            template_flat = empty_token.view(1, word_dim).repeat(num_preds * num_words, 1)
        
            valid_mask_zero_pad = jt.concat([jt.zeros_like(valid_mask[:, :1]),
                                            valid_mask], dim=-1)
       
            pe_indices = (valid_mask_zero_pad > 0.0).cumsum(dim=-1)[:, :-1]
            pe_indices_flat = (pe_indices + (jt.arange(num_preds
                                                     )
        ...
        else: end_token_ids = (jt.ones(num_preds)*(num_words-1)).long()
...
def build_TES_model(state_dict: dict,  n_ctx: int=None, use_proj: bool=True):
    ...
    model = TES_CLIP(embed_dim, image_resolution, vision_layers, vision_width,
                 vision_patch_size, context_length, vocab_size,
                 transformer_width, transformer_heads, transformer_layers,n_ctx,use_proj)
